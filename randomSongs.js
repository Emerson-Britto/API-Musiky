...
const randomSongs = async (totalList, resultPerList) => {
    ...
    const { data } = await axios.get(`https://youtube.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=999&playlistId=PLmqZgWL_h1i9TZTr6AhRhEkXSKCTvIdMr&key=AIzaSyDmGmrj5u-0g`)
    for (var i = 0; i < 9; i++) {
...
const randomSongs = async (totalList, resultPerList) => {
	...
	while(nextPag){
		...
		const { data } = await axios.get(`https://youtube.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=999${nextPagToken}&playlistId=PLmqZgWL_h1i9TZTr6AhRhEkXSKCTvIdMr&key=AIzaSyDmGm--tEvrj5u-0g`)
